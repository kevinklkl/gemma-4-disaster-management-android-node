...
object LlamaEngine {
    ...
    internal external fun generate(ctxHandle: Long, prompt: String, temperature: Float, maxTokens: Int): String
    private external fun freeContext(ctxHandle: Long)
    private external fun freeModel(modelHandle: Long)

    fun load(modelPath: String, nGpuLayers: Int = 0): Boolean {
        modelHandle = loadModel(modelPath, nGpuLayers)
        if (modelHandle == 0L) return false
        contextHandle = createContext(modelHandle, 2048)
